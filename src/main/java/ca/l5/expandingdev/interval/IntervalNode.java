...
public class IntervalNode {
    ...
    private enum Direction {LEFT, RIGHT} //二叉树
    ...
    public IntervalNode(final List<Intervalable> intervals) {
        ...
        for (Intervalable interval : intervals) {//这里貌似是在建树，根据中间量point进行划分左右子树。
    ...
    public int determineMedian(final List<Intervalable> intervals) {//找所有interval的中间下标
        int start = -1;
        int end = -1;
    ...
    //从这一行开始往下，全是Overlap(重叠)的相关内容，暂时不看
