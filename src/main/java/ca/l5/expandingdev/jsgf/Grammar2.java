...
public class Grammar {
    ...
    public List<Rule> getMatchingRule(String test, Set<Integer> ruleIndexFiltered) throws Exception {
        clear_result();
        List<Rule> resultList = new ArrayList<>();
        for (int k =0;k<rules.size();k++) {
            Rule r = rules.get(k);
            if(!ruleIndexFiltered.contains((int)r.index)){
                continue;
            }
//            System.out.println("[Grammar][getMatchingRule]待匹配ruleIndex:"+r.index+"待匹配rule:"+r.getRuleString());
            if (!r.isWild) {
                if (matchesRule(r,test)) {
                    ...
                    for (int j = 0 ; j < re_tags.size(); j += 1) {//打印抽槽 **************
                        ...
                        if (tstart < tend) {//防止空的出现
                            String tmp1 = re_tags.get(j).getString();
                            String tmp2 = test.substring(tstart,tend).replaceAll(" ","");
                    ...
                    // TODO 确认优先级提取，目前暂时都设为1.0
                    Rule subMatched = new Rule();
                    subMatched.setUtterance(r.name);
                    subMatched.setSlots(slotList);
                    subMatched.setIndex(k);
                    subMatched.setPriority(1.0f);
                    resultList.add(subMatched);
                    resultList.add(subMatched);
                }
                ...
                if (r.wl.size() == 2) {//一个通配符
                    for (int i = r.wl.get(0); i <= r.wl.get(1); i += 1) {
                        if (matchesRule_w(r, i, test)) {
                            ...
                            TemplateSubMatched subMatched = new TemplateSubMatched();
                            subMatched.setUtterance(r.name);
                            subMatched.setSlots(slotList);
                            subMatched.setIndex(k);
                            subMatched.setPriority(1.0f);
                    ...
                    for (int i = r.wl.get(0); i <= r.wl.get(1); i+=1) {//这种循环逻辑下，优先第一个W的贪婪匹配，但第二个W就不一定贪婪了
                        for (int j = r.wl.get(2); j <= r.wl.get(3); j+=1) {
                            ...
                            if (matchesRule_ww(r, wl, test)) {
                                List<Slot> slotList = new ArrayList<>();
                                for (int m = 0 ; m < re_tags.size(); m += 1) {//打印抽槽 **************
                                    //System.out.println(re_tags.get(i).wp+" "+re_tags.get(i).wpe);
                                    int tstart = 2*re_tags.get(m).wp;
                                    int tend = 2*re_tags.get(m).wpe-1;
                                    if (tstart < tend) {//防止空的出现
                                        String tmp1=re_tags.get(m).getString();
                                        String tmp2=test.substring(tstart,tend).replaceAll(" ","");
                                        Results tmpr=new Results(tmp1,tmp2);
                                        results.add(tmpr);
                                        Slot slot = new Slot();
    ...
    public static Grammar parseGrammarFromString(String s) throws IOException {//将字符串解析成语法对象
        dict = loadFileToMap("src/main/test/template-dict.txt");
        dict2 = reverse(dict);
...
//        dict2 = reverse(dict);
            ...
            for (String statement : statements) {
                ...
                if (statement.startsWith("grammar ")) {
                    ...
                    if (ruleName.contains("#")) {//提取优先级，简化rule的名字，用对象属性index加以区分
                        ...
                        ruleName = ruleName.substring(indexJing);
                    } else {
                        int indexAnd = ruleName.indexOf('&');
                        ruleName = ruleName.substring(indexAnd);
                    }
    ...
    /**
     * 根据query，利用AC自动机搜索keyword
     * @param s
     * @return
     */
    public LinkedHashMap<Integer, List<ACResult>> ahoCor(String s) {
        String ss = s.replace(" ", "");
        ...
        while (iterator.hasNext()) {
            ...
            WordKeyBO wordKeyBO = new WordKeyBO(keyword,lable);
            ACResult acr = new ACResult(wordKeyBO,end+1);
            if (re == null) {
    ...
    public static void ahoCor_pre(Map<String, ArrayList<String>> slots_list) {
        PayloadTrie.PayloadTrieBuilder<AcSlot> trie_builder = PayloadTrie.builder();
        for (Map.Entry<String, ArrayList<String>> entry : slots_list.entrySet()) {
     ...
     * @return
     */
    public Set<Integer> findUnionSetForWordList(List<List<WordKeyBO>> wordList, Map<String, Set<Integer>> key2IndexMap) {
        ...
        for(List<WordKeyBO> sentence: wordList){
            ...
            if(sentence!=null&&!sentence.isEmpty()){
                ...
                Set<Integer> firstWordIndexSet = getWordIndexUnionSet(firstWord.getLabelList(), key2IndexMap);
                ...
                for(int i=1;i<sentence.size();i++){
                    ...
                    if(nextWord.getValue()!=null&&!wordSet.contains(nextWord.getValue())){
                        ...
                        Set<Integer> nextWordIndexSet = getWordIndexUnionSet(nextWord.getLabelList(), key2IndexMap);
     ...
     * @return
     */
    private static Set<Integer> getWordIndexUnionSet(List<String> labelList, Map<String, Set<Integer>> key2IndexMap){
